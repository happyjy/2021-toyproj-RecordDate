...
// dycalendar객에 date 설정3: month click
// dycalendar객에 date 설정1 - date click

const cycalendar = (function (global) {
  // 'use strict';
  ...
  var //this will be used by the user.
    dycalendar = {
      set setCurrentYear(year) {
        ...
        this.today = makeFormatDate(year, this.currentMonth, this.currentDate);
      },
      set setCurrentMonth(month) {
        this.currentMonth = month;
        this.today = makeFormatDate(this.currentYear, month, this.currentDate);
      },
      set setCurrentDate(date) {
        this.currentDate = date;
        this.today = makeFormatDate(this.currentYear, this.currentMonth, date);
      },
      currentYear: year,
      currentMonth: month,
      currentDate: date,
  ...
  function drawCalendarMonthTable(data, option) {
    ...
    // dycalendar.currentDate = option.date;
    // dycalendar.currentMonth = option.month;
    ...
    elem.dataset.year = data.month;
    elem.dataset.monthName = data.year;
    ...
    } else if (option.monthformat === 'ddd') {
      elem.innerHTML = `${data.year}년 ${data.month + 1}월`;
    }
  ...
  function onClick() {
    container.onclick = function (e) {
      ...
      if (
        ...
        // option.date = date;
        option.date = dycalendar.currentDate;
        option.month = month;
        option.year = year;
      ...
      // date click
      if (targetDomObject?.classList?.contains('calendarDate')) {
        // const formatDate = makeFormatDate('2021', '11', '12');
        // del highlight
        highlightDom.classList.remove('dycalendar-target-date');

        // highlight click date
        targetDomObject.classList.add('dycalendar-target-date');
        highlightDom = targetDomObject;
  ...
  dycalendar.draw = function (option) {
    ...
    // const makeFormatDate = function (Y, M, D) {
